...
// Note: We require the functions that define the models, but don't initialize them yet.
// Initialization happens later when the sequelize instance is available.
const MappingConfigModelDefinition = require('./models/mappingconfig');
const ConnectorConfigModelDefinition = require('./models/connectorconfig');
const ApplicationModelDefinition = require('./models/application');
...
// Pass the sequelize instance and DataTypes to the model definition functions.
const MappingConfig = MappingConfigModelDefinition(sequelize, DataTypes);
const ConnectorConfig = ConnectorConfigModelDefinition(sequelize, DataTypes);
const Application = ApplicationModelDefinition(sequelize, DataTypes);
...
   } else {
      console.log(`[ConfigService] Cache key not found, nothing to clear: ${key}`);
   }
}
...
    console.log(`[ConfigService] Loaded service config for ${serviceName}. Found: ${!!configEntry}.`);
...
 * @param {string | null} [sourceId=null] - Optional associated source entity ID (Application or ConnectorConfig).
...
    console.log(`[ConfigService] Loaded mapping config "${name}" (SourceType: ${sourceType}, TargetType: ${targetType}). Found: ${!!mappingConfig}`);
    return mappingConfig; // Return the Sequelize instance or null

  } catch (error) {
    console.error(`[ConfigService] Error loading mapping config "${name}" (${sourceType} -> ${targetType}):`, error);
    throw new Error(`Failed to load mapping config "${name}" (${sourceType} -> ${targetType}): ${error.message}`);
  }
};
...
    console.log(`[ConfigService] Loaded connector config "${name}" (ServiceType: ${serviceType}, Type: ${type}). Found: ${!!connectorConfig}`);
    return connectorConfig; // Return the Sequelize instance or null

  } catch (error) {
    console.error(`[ConfigService] Error loading connector config "${name}" (${serviceType}/${type}):`, error);
    throw new Error(`Failed to load connector config "${name}" (${serviceType}/${type}): ${error.message}`);
  }
};
...
  clearCache,           // Export specific cache clearing function
  clearAllCache,        // Export all cache clearing function (useful for testing)
  mappingCacheKey       // Export mappingCacheKey function for generating keys externally
