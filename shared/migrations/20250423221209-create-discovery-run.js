// In your YYYYMMDDHHMMSS-create-discovery-run.js file
'use strict';

/** @type {import('sequelize-cli').Migration} */
module.exports = {
  async up(queryInterface, Sequelize) {
    await queryInterface.createTable('DiscoveryRuns', {
      ...
      applicationId: {
        type: Sequelize.UUID,
        allowNull: false, // A Discovery run is for a specific Application
        // --- Foreign Key Constraint (add this) ---
        references: {
          model: 'Applications', // References the Applications table
          key: 'id',
        },
        onUpdate: 'CASCADE', // Update foreign key if referenced app id changes
        onDelete: 'CASCADE' // Delete this entry if the application is deleted (less useful to keep the log)
  ...
  async down(queryInterface, Sequelize) {
    ...
    // await queryInterface.removeConstraint('DiscoveryRuns', 'DiscoveryRuns_applicationId_fkey'); // Example constraint name
    await queryInterface.dropTable('DiscoveryRuns');
  }
};